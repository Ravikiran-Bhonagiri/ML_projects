...
# The more images used in training the better. Normally a lot of images are used for
# training a face recognizer so that it can learn different looks of the same person,
# for example with glasses, without glasses, laughing, sad, happy, crying, with beard,
# without beard etc. To keep our tutorial simple we are going to use only 12 images for each person.
#
# So our training data consists of total 2 persons with 12 images of each person.
# All training data is inside _`training-data`_ folder. _`training-data`_ folder contains one
# folder for each person and **each folder is named with format `sLabel (e.g. s1, s2)` where
# label is actually the integer label assigned to that person**. For example folder named s1 means
# that this folder contains images for person 1. The directory structure tree for training data is as follows:
#
# ```
# training-data
# |-------------- s1
...
# 2. For each subject, extract label number. **Do you remember that our folders have a special
#       naming convention?** Folder names follow the format `sLabel` where `Label` is an integer
#       representing the label we have assigned to that subject. So for example, folder name `s1`
#       means that the subject has label 1, s2 means subject label is 2 and so on. The label extracted in this step is assigned to each face detected in the next step.
...
cv2.imshow( "-----------" , cv2.resize(predicted_img1, (400, 500)))
cv2.imshow( "------------", cv2.resize(predicted_img2, (400, 500)))
